  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      startTransition(async () => {
        const res = await importCsvTransactions(content, file.name)
        setResult(res)
    ...
    <div className="p-6 space-y-6 max-w-xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          ...
          >
            <History className="h-4 w-4" /> Historia plików
          </Link>
          <Link
            href="/import/history"
            className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
        ...
        <div className="flex items-center gap-4">
          ...
          {lastImport?.created_at && (
            <span className="text-xs text-muted-foreground whitespace-nowrap">
              Ostatni import: {formatDateTime(lastImport.created_at)}
            </span>
          )}
        </div>
          ...
          <div className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/30 p-3 rounded-md border border-blue-200 dark:border-blue-900 mt-4">
            ...
            <div>
              ...
              {(lastImport.originalFileName || lastImport.savedFileName) && (
                <div className="text-xs opacity-80 mt-0.5">Zapisany jako: {lastImport.originalFileName || lastImport.savedFileName?.replace('import_', '').substring(0, 8) + '...'}</div>
              )}
            </div>
          </div>
        )}
