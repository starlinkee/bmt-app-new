...
log "3/3 Testy e2e (GitHub Actions) na świeżym deploymencie preview tego commita"
if ! command -v gh >/dev/null 2>&1 || ! gh auth status >/dev/null 2>&1; then
  ...
  [ -n "$RUN_ID" ] || fail "Nie wystartowały testy e2e w GitHub Actions dla $SHA (deploy preview nieudany lub nie gotowy po 10 min)."
  if ! gh run watch "$RUN_ID" --exit-status; then
    fail "Testy e2e NIE przeszły: $(gh run view "$RUN_ID" --json url --jq .url). Napraw je zanim spróbujesz znowu."
  fi
fi
